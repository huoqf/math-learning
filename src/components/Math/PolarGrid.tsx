...
export const PolarGrid: React.FC<PolarGridProps> = ({
  ...
  const toDesign = (mx: number, my: number) =>
    mathToDesign(mx, my, scale)
  ...
  const circles = React.useMemo(() => {
    ...
    for (let r = radiusStep; r <= maxRadius; r += radiusStep) {
      ...
      for (let i = 0; i <= steps; i++) {
        ...
        segments.push(`${i === 0 ? 'M' : 'L'} ${pt.x.toFixed(1)} ${pt.y.toFixed(1)}`)
      }
      result.push(
        <path
          key={`circle-${r}`}
  ...
  return (
    <g>
      {/* 主轴 */}
      <line
        x1={xAxisLeft.x} y1={xAxisLeft.y}
        x2={xAxisRight.x} y2={xAxisRight.y}
        stroke={axisColor} strokeWidth={1.5}
      />
      <line
        x1={yAxisTop.x} y1={yAxisTop.y}
        x2={yAxisBottom.x} y2={yAxisBottom.y}
        stroke={axisColor} strokeWidth={1.5}
      />
