 ...
 * 三角函数图象变换纯计算模块
 *
 * 学段约定（对应审查报告 P1「ω < 0 未声明」）：
 *   本模块所有关于周期 T = 2π/ω、单调区间、对称轴/对称中心与五点作图的结论，
 *   均在 **ω > 0** 的前提下成立。参数面板已将 ω 约束在 [0.2, 6]（恒正），
 *   故界面不会出现 ω < 0 的情形；如后续放开该约束，需先按
 *   y = A sin(ωx+φ) = A sin(-|ω|x+φ) 用诱导公式化为正频率形式再复用本模块，
 *   否则单调区间方向会整体反向。
 */
