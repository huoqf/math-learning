...
export function getModeTipConfig(ctx: BayesScenarioCtx): TipConfig {
  ...
  if (activeMode === "total_prob") {
    ...
    if (totalScenario === "balanced") {
      // 预设置为 P(A₁)=P(A₂)=0.33、P(A₃)=0.34（由 pA1/pA2 联动推导）。
      // 受左屏滑块步长 0.05 限制，1/3 无法精确取到，故文案必须与真实取值一致：
      // 旧文案写"三者均等于 1/3"，与中屏 Treemap 三块不等宽直接冲突（审计 P2-2）。
      return {
        variant: "info",
        badge: "高考模型 · 近似三等分均衡加权模型",
        background:
          "近似等可能完备情境：各原因分支先验概率接近等可能发生（如近似等概率抽取三个不同的盒子）。",
        condition:
          "三个原因分支先验概率近似均等：$P(A_1) = P(A_2) = 0.33$、$P(A_3) = 0.34$（受滑块步长 $0.05$ 限制，取最接近 $\\frac{1}{3}$ 的一组）。",
        question:
          "证明当各分支先验概率近似等可能时，全概率加权和退化为各分支条件概率的算术平均数 $\\frac{1}{3} \\sum_{i=1}^3 P(B|A_i)$，并求出它与真实加权和 $\\sum_{i=1}^3 P(A_i)P(B|A_i)$ 的偏差。",
      };
    }
    if (totalScenario === "warner") {
      return {
