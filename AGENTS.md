...
- `ParamControl`（消除双重边框与多重嵌套）：
  - 默认采用嵌入模式（`embedded`），**严禁在 Section 内嵌套自带外框的 ParamControl（杜绝双白框、双边框、双重标题）**；
  - 严禁在分组（`group`）时嵌套子卡片（杜绝 `bg-neutral-50 rounded-xl p-3.5`），统一使用扁平指示线以保证 100% 滑轨宽度；
  - 自解释参数直接**省略 `description`**，压缩垂直空间，让界面紧凑自然；
- `SelectGrid`：
  - 采用纯净加粗中文名称（如“医药临床试验”、“教学达标实验”），自解释选项**省略 `description`**；
  - ❌ **严禁**在选项副标题中堆砌冰冷代码变量（如 $a=85, b=15...$）或生硬中间式（如 $\lambda=-0.5, P(D)=2\%$）；
  - ✅ **双列紧凑排版**：预设选项 $\ge 4$ 项时，优先采用 `columns={2}` 双列网格排版，移除不必要的 `fullWidth`，垂直高度压缩 50%，实现左屏免滚动；
- `TipCard`（教学导引题设化）：
  - 必须置于左屏最底部，承载完整的题设解释职责；
  - 推荐采用结构化 Props：`badge`（高考模型徽章）+ `condition`（【初始条件】）+ `question`（【核心设问】）；
  - 严禁在 TipCard 提前剧透推导过程或最终数值答案（推导 100% 归位右屏 `MathPanel`）。
...
* **预设与典型情景的本质（参数降维与题设锁定，杜绝认知混乱）**：
  ...
  - **数形双向联动与右屏同步**：
    - 中屏拖拽几何动点时，系统**自动切回【自由探索】（`free`）**并展开全量参数；
    - 右屏 `MathPanel` 的【公式定理 `theorems`】与【高考要点 `gaokaoPoints`】**随二级情景 100% 动态置顶特化**（如选独立置顶乘法判定，选互斥置顶加法公式，选传球置顶不动点构造等比数列法）。

#### 3. 参数对象化聚合与纯净性规范（全学科通用）
* **对象化分组**：同一几何对象（如定点 $P_0(x_0, y_0)$、向量分量 $(v_x, v_y)$、复数实虚部、概率完备划分与分支概率）配置相同的 `group` 属性，实现主次分层，严禁散落平铺；
