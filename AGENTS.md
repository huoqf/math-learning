...
**左屏结构与参数动态裁剪铁律**（全项目所有页面必须遵守）：
1. **统一层级结构**：
   - 探究模式 (`SelectGrid`) $\to$ 典型预设 (`SelectGrid`) $\to$ 参数调节 (`ParamControl`) $\to$ 图层开关 (`Toggle`) $\to$ 视角/视图预设
2. **“探究模式 + 典型预设”黄金 2×2 规范**：
   - 预设网格第一项必须为【自由探究】（`key: "free"`，默认选中），描述为“全参数开放”；
   - 预设选项统一采用 2×2 对称网格（4 项：1 项自由探究 + 3 项典型预设），微描述 $\le 6$ 字，杜绝省略号截断。
3. **参数动态裁剪与几何约束联动（教学目标驱动）**：
   - **自由探究下**：该模式所涉及的全部合法参数全量展开；
   - **选择特定预设后**：
     - **几何对称/退化约束**（如正方体、正棱柱、等边三角形、圆）：锁定对称等长关系，仅展示单个基准主参数，隐藏其余从属参数并自动同步联动；
     - **特征固定点/固定构型**（如中点 $\lambda=0.5$、顶点极值 $\lambda=1.0$、特殊切点）：隐藏被锁定的特征点滑块，防止学生误调导致名不副实，仅展示几何体尺寸或其他自由维度；
4. **数形双向联动解耦**：
   - 当学生在画布（2D SVG / 3D Canvas）中直接拖拽动点时，系统**自动将预设状态切回【自由探究】（`free`）**，并立即全量展开所有参数滑块。
5. **左屏纯净性与三屏职责**：
   - 左屏仅承载交互控制，**严禁在左屏堆砌高考考点总结、定理公式推导或大段教学解释文字**（100% 归位右屏 `MathPanel`）。
6. **严禁无信息量操作废话**：
   - 严禁在左屏放置类似“鼠标旋转视角、滚轮缩放”等通用软件常识提示。
7. **参数对象化聚合与 group 命名铁律**：
   - 属于同一几何对象（如定点 $P_0(x_0, y_0)$、弦中点 $M(x_0, y_0)$、向量分量 $(v_x, v_y)$、复数实虚部 $(a, b)$）或同一逻辑层级的参数，必须在 `ParamConfig` 中配置相同的 `group` 属性（如 `"定点 P₀(x₀, y₀) 坐标"`、`"直线方向与动点参数"`、`"椭圆几何底模"`），由组件原生实现自然聚合与主次分层，严禁散落平铺。
