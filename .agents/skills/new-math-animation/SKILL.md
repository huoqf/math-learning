...
- **左屏层级动线与分科原则** → 
  - **代数/函数/数列基础专题 (A类概念课)**：`探究模式 (TabSwitcher) → 核心参数调节 (ParamControl) → 概念导引 (TipCard)`，动线纯净，严禁无脑生硬堆砌预设，右屏装配定理与性质；
  - **多构型/解析几何/概率统计/导数综合 (B类高考专题)**：
    - `SelectGrid`：采用纯净加粗中文名称，预设 $\ge 4$ 项时配置 `columns={2}` 双列网格，垂直空间压缩 50%；**严禁配置 formula 堆砌公式**，自解释选项省略 `description`，严禁在副标题中堆砌代码变量或生硬中间式；
...
- **全屏三屏符号一致性 (SSOT 铁律)** → 
  - 左屏滑块标签必须采用三位一体（`\text{几何/代数含义 } \color{Token}{代号}`）；
  - 函数切点用 $x_0$、几何棱长用具体线段端点对（如 $PA=a$）、向量用 $\vec{a}, \vec{b}$、圆锥曲线用 $e, k, m$、概率用 $P(A), n$；
  - 中屏图元线段与右屏推演代入公式必须 1-to-1 带入相同代号，主控红、从属橙、高维绿三屏严格同色。
- **全屏数学表达式 KaTeX 规范** → 严禁在普通文本/description 中直接裸写 `e^{x-1}` 或 `\ln x` 等带花括号 `{}` 源码；TipCard 等区域必须使用 `<KatexFormula formula="..." mode="inline" />` 渲染；`SelectGrid` 严禁堆砌公式。
