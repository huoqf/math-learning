...
- 扫描范围：`builders` / `registries` / `knowledgeTree` / `meta.ts` / `modeConfig` / `Animation.tsx` / `Page.tsx` / `Scene.tsx`，以及 **`src/features` 下全部非测试源码**（含首页知识树卡片等文案载体，避免超纲词从"第一屏"漏网）；
- 命中黑名单术语（洛必达 / 麦克劳林 / 泰勒 / 琴生 / 凹凸 / 极点极线 / 克拉默 / 外积 / 叉积 / 夹逼 / 等价无穷小 / 上确界 / 下确界 / 紧致 / 数列极限 / 特征方程 / 马尔可夫链 / 平稳分布 / 卡方分布 / 概率密度函数 / 微元 / 定积分 / 极坐标 / 参数方程 …）且**未在条目级声明拓展** → **error（阻断）**；
- **豁免为「条目级」**（不再是文件级）：只有下面两条之一成立才降级为 warning——
  1. 命中行所属的**数据条目对象**在**自身直接属性**上写了 `isExtension: true` / `importance: "extend"` / `status: "拓展"|"选学"|"竞赛"`（函数体、`if` 体、`return { … }` 这类外层聚合对象都**不算**条目）；
  2. 该文件**所属知识树节点** `importance === "extend"`（`src/features/<f>/**` 取同目录 `meta.ts`；`src/data/builders/<b>.ts` 经 `mathQuantities.ts` 的 `animId → builder` 反查节点）。
- **自由文本不再构成豁免**：`超出课标` / `选学` / `拓展 ·` / `（拓展` 这类字样出现在文案里**一律不豁免**。历史漏洞：旧实现只要文件里任意一处出现「拓展」字样就整份文件豁免，导致"改个词、或把术语降为注释"即可绕过门禁，并会把同文件其余未标注条目一并放行。现在必须逐个条目显式标注。

因此，写拓展内容时的标准姿势是：**内容保留 + 在该条目上标注 `isExtension: true`（右屏定理/警示）或让所属知识树节点 `importance: "extend"` + 页面显示「拓展 · 超出课标」徽标**，而不是删除。注意徽标只是展示，**不构成**门禁豁免依据。

> 回归测试见 `src/test/auditEngine.test.ts` → 「超纲术语门禁：文件级豁免 → 条目级豁免」（7 例，覆盖兄弟条目外溢、自由文本失效、节点级豁免正反向）。

> **配套说明（解答题规范与学科边界）**：在数列递推通项收敛与离散分布列大样本逼近等高考解答题场景中，高中课标解答题卷面未定义数列极限记号，严禁直接书写未定义的 $\lim_{n \to \infty} p_n$ / $\lim_{N \to \infty}$，必须改写为「随着项数增大趋近于定值」或「$N$ 远大于 $n$ 时近似相等」等规范文字化表述，并由单测严格守住；而在人教A版课标正文内的导数概念定义式（瞬时变化率极限）以及显式声明拓展的高等数学专题（如洛必达法则、无穷级数求和）中，极限记号属于合法内容，正常按规范使用。
